...
const INITIAL_CHECKOUT_ITEMS: CheckoutItem[] = [
    {
        id: 'jewel-01',
        name: 'The Celeste Solitaire Brilliant Cushion-Cut Diamond Ring',
        category: 'Rings',
    ...
    {
        id: 'jewel-02',
        name: 'Aurelia Hand-Woven 18k Filigree Chain Link Necklace',
        category: 'Necklaces',
...
export default function CheckoutPage() {
    ...
    // Price Calculations
    const totalQuantityCount = items.reduce((acc, current) => acc + current.quantity, 0);
    const subtotalSum = items.reduce((acc, current) => acc + (current.available ? current.price * current.quantity : 0), 0);
    const calculatedShippingCost = subtotalSum > 200000 || subtotalSum === 0 ? 0 : 500;
    const calculatedTaxPlaceholder = Math.round(subtotalSum * 0.03); // 3% GST/Tax 
    const finalAggregatedTotal = subtotalSum - appliedDiscount + calculatedShippingCost + calculatedTaxPlaceholder;
    ...
    const executeCouponValidation = (e: React.FormEvent) => {
        e.preventDefault();
        if (!couponCode.trim()) return;

        const standardCode = couponCode.trim().toUpperCase();
        if (standardCode === 'JEWEL10') {
            if (subtotalSum < 500000) {
                setCouponStatus('unmet');
                setAppliedDiscount(0);
            } else {
                setCouponStatus('valid');
                setAppliedDiscount(50000);
            }
        } else if (standardCode === 'GOLD') {
                                ...
                                <div className="lg:col-span-5 lg:sticky lg:top-8 space-y-6">
                                    ...
                                    <div className="bg-[var(--color-bg-secondary)] border border-[var(--color-border)] p-6 rounded-[var(--radius-sm)] shadow-md space-y-6">
                                        ...
                                        <button
                                            ...
                                            onClick={() => alert('Processing your secure payment transaction...')}
