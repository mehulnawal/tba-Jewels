...
export function AuthModal({ isOpen, onClose }: LuxuryAuthModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        console.log(`Submitting ${authMode} for:`, { email, password, name });
        onClose();
    };
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            {/* Modal Box Container (Using your genuine app backgrounds & borders) */}
            <motion.div
            ...
            >
                ...
                {/* Header Branding (Fixed Brand Title to TBA & spacing variables) */}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {authMode === 'login' && (
                        <div className="text-right">
                            <a href="#forgot" className="font-secondary text-[11px] text-[var(--color-text-muted)] hover:text-[var(--color-teal)] transition-colors">
                                Forgot password?
                            </a>
                        </div>
                    )}

                    {/* Action Execution Button (Using your primary var(--color-teal) variables) */}
                    <button
                        type="submit"
                        className="w-full bg-[var(--color-teal)] text-[var(--color-cream)] font-secondary text-xs uppercase tracking-widest py-3.5 mt-4 transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer group hover:opacity-90"
                    >
                ...
                {/* Bottom Toggle Mode Trigger Link */}
